...
export function nextStep(): void {
    ...
    Object.keys(currentState.persons).forEach((id) => {
        people[id] = createPerson(currentState.persons[id]);
    });

    const locations: { [key: string]: ILocation } = {};
    Object.keys(currentState.locations).forEach((id) => {
        locations[id] = createLocation(currentState.locations[id]);
    });
    ...
    Object.keys(people).forEach((person) => {
        stage1(people[person], step_info);
    });
